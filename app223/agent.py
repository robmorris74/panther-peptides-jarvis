...
def _chat_request(model,messages,tools=None,timeout=120):
 ...
 key=os.getenv('OPENAI_API_KEY','').strip();req=urllib.request.Request('https://api.openai.com/v1/chat/completions',data=json.dumps(payload).encode(),headers={'Authorization':f'Bearer {key}','Content-Type':'application/json','User-Agent':'Jarvis-v223'})
...
def _model_call(messages,tools=None):
 ...
 for model in _models():
  for attempt in range(3):
   try:
    out=_chat_request(model,messages,tools);_active_model=out.get('model') or model;_last_model_error='';return out
...
def model_status(probe=False):
 ...
 try:
  out=_model_call([{'role':'user','content':'Reply with exactly: JARVIS_MODEL_OK'}]);base.update({'ok':True,'active_model':out.get('model'),'probe_text':out.get('content','')[:120]})
...
def _run(oj):
 oid=oj['id'];_beat();messages=[{'role':'system','content':'You are Jarvis v223, a persistent autonomous operations and engineering agent. Diagnose, plan, act, observe, verify, conclude. Never claim success without concrete verification. Use live tools for system state. Do not send external communications, release quarantined inventory, change credentials, or perform irreversible/high-impact actions without explicit owner approval.'},{'role':'user','content':oj['detail']}];execute("UPDATE objectives SET state='running',last_error=NULL,blocked_reason=NULL,updated_at=CURRENT_TIMESTAMP WHERE id=?",(oid,))
 for step in range(1,int(oj.get('max_steps') or 30)+1):
  ...
  if out['tool_calls']:
   ...
   for tc in out['tool_calls']:
    ...
    try:result=run_tool(tc['function']['name'],args);log(oid,'tool_result',json.dumps(result)[:6000])
    except Exception as e:result={'error':str(e)};log(oid,'tool_error',str(e),'error')
    messages.append({'role':'tool','tool_call_id':tc['id'],'content':json.dumps(result)})
   continue
  text=(out['content'] or '').strip() or 'Objective completed.';log(oid,'agent_result',text);low=text.lower()
  if any(x in low for x in ('need owner approval','owner approval required','genuinely blocked','cannot proceed without')):execute("UPDATE objectives SET state='blocked',blocked_reason=?,updated_at=CURRENT_TIMESTAMP WHERE id=?",(text[:3000],oid));return
  execute("UPDATE objectives SET state='completed',completed_at=CURRENT_TIMESTAMP,updated_at=CURRENT_TIMESTAMP WHERE id=?",(oid,));return
...
def start():
 ...
 _stop.clear();_beat()
 if not _thread or not _thread.is_alive():_thread=threading.Thread(target=_loop,name='jarvis223-supervisor',daemon=True);_thread.start()
 if not _watchdog or not _watchdog.is_alive():
  def wd():
   global _thread
   while not _stop.wait(5):
    if not _thread or not _thread.is_alive():_thread=threading.Thread(target=_loop,name='jarvis223-supervisor',daemon=True);_thread.start()
  _watchdog=threading.Thread(target=wd,name='jarvis223-watchdog',daemon=True);_watchdog.start()
def wake():_wake.set();return status()
def status():
 try:q=one("SELECT COUNT(*) n FROM objectives WHERE state='queued'")['n'];r=one("SELECT COUNT(*) n FROM objectives WHERE state IN ('planning','running')")['n'];b=one("SELECT COUNT(*) n FROM objectives WHERE state='blocked'")['n'];c=one("SELECT COUNT(*) n FROM objectives WHERE state='completed'")['n'];db_ok=True
 except Exception:q=r=b=c=0;db_ok=False
 return {'worker_alive':bool(_thread and _thread.is_alive()),'watchdog_alive':bool(_watchdog and _watchdog.is_alive()),'heartbeat_age_seconds':round(max(0,time.time()-_heartbeat),1) if _heartbeat else None,'queued':q,'running':r,'blocked':b,'completed':c,'database_ok':db_ok,'last_loop_error':_last_loop_error,'model':model_status(False)}
